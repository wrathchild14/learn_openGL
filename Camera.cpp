...
void Camera::Inputs(GLFWwindow* window)
{
    ...
    {
        m_Position += m_Speed * -normalize(cross(m_Orientation, m_Up));
    }
    ...
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS)
    {
        ...
        glm::vec3 newOrientation = rotate(m_Orientation, glm::radians(-rotX), normalize(cross(m_Orientation, m_Up)));

        // Decides whether or not the next vertical Orientation is legal or not
        if (abs(angle(newOrientation, m_Up) - glm::radians(90.0f)) <= glm::radians(85.0f))
        {
            m_Orientation = newOrientation;
        }

        // Rotates the Orientation left and right
        m_Orientation = rotate(m_Orientation, glm::radians(-rotY), m_Up);
